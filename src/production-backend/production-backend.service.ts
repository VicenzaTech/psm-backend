...
export class ProductionBackendService {
  ...
  private async request<T>(method: HttpMethod, path: string): Promise<T> {
    ...
    try {
      ...
      this.logger.error(
        `Request to production backend failed: ${url}`,
        error?.stack,
      );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Production backend request failed with status ${response.status}`,
      );
    }
    ...
    try {
      ...
      this.logger.error(
        `Failed to parse JSON response from production backend for ${url} with data: ${response.body}`,
        error?.stack,
      );
  ...
  async getDeviceByDeviceId(
    deviceId: string,
  ): Promise<BRICK_COUNTER_DEVICE | null> {
    if (!deviceId) return null;
    const encoded = encodeURIComponent(deviceId);
    return this.request<BRICK_COUNTER_DEVICE | null>(
      'GET',
      `/internal-api/devices/by-device-id/${encoded}`,
    );
  }

  async getMeasurementTypeById(
    id: number,
  ): Promise<BRICK_COUNTER_MEASUREMENT_TYPE | null> {
    if (id == null) return null;
    return this.request<BRICK_COUNTER_MEASUREMENT_TYPE | null>(
      'GET',
      `/internal-api/measurement-types/${id}`,
    );
  }

  async getPositionById(
    id: number,
  ): Promise<(BRICK_COUNTER_POSITION & { productionLineId?: number }) | null> {
    if (id == null) return null;
    return this.request<
      (BRICK_COUNTER_POSITION & { productionLineId?: number }) | null
    >('GET', `/internal-api/positions/${id}`);
  }

  async getProductionLineById(
    id: number,
  ): Promise<BRICK_COUNTER_PRODUCTION_LINE | null> {
    if (id == null) return null;
    return this.request<BRICK_COUNTER_PRODUCTION_LINE | null>(
      'GET',
      `/internal-api/production-lines/${id}`,
    );
  }
}
