...
export class StageDeviceMappingController {
  constructor(
    private readonly stageDeviceMappingService: StageDeviceMappingService,
  ) {}
  @Get()
  ...
  async getByProductionLine(
    @Param('productionLineId', ParseIntPipe) productionLineId: number,
  ) {
    return await this.stageDeviceMappingService.getMappingsByProductionLine(
      productionLineId,
    );
  }
  ...
  async updateDeviceMapping(
    ...
    @Body() body: UpdateStageDeviceMapping,
  ) {
  ...
  async updateStageStatus(
    @Param('id', ParseIntPipe) id: number,
    @Body() body: UpdateStageDevieMappingStatus,
  ) {
