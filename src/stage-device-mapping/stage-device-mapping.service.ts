...
export class StageDeviceMappingService {
  ...
  async createDeviceMapping(dto: CreateStageDeviceMapping) {
    ...
    if (!productionLine.isActive) {
      throw new BadRequestException(
        'Cannot map device to an inactive production line',
      );
    }

    // Only 1 active mapping per device
    const existingDeviceMapping =
      await this.prisma.stageDeviceMapping.findFirst({
        where: {
          iotDeviceId,
          isActive: true,
        },
    ...
    const [device, measurementType, position] = await Promise.all([
      ...
      this.productionBackend.getMeasurementTypeById(
    ...
    if (!device) {
      throw new BadRequestException(
        'iotDeviceId does not exist on production backend',
      );
    }

    if (!measurementType) {
      throw new BadRequestException(
        'iotMeasurementTypeId does not exist on production backend',
      );
    }

    if (!position) {
      throw new BadRequestException(
        'measurementPosition does not exist on production backend',
      );
    }
  ...
  async updateDeviceMapping(
    stageMappingId: number,
    dto: UpdateStageDeviceMapping,
  ) {
    const {
      iotDeviceId,
      iotMeasurementTypeId,
    ...
    const targetProductionLineId =
      typeof productionLineId === 'number'
        ? productionLineId
    ...
    if (!productionLine) {
      throw new NotFoundException(
        `Production line with id ${targetProductionLineId} not found`,
      );
    }

    if (!productionLine.isActive) {
      throw new BadRequestException(
        'Cannot update mapping on an inactive production line',
      );
    }
    ...
    if (iotDeviceId) {
      const device =
        await this.productionBackend.getDeviceByDeviceId(iotDeviceId);
      if (!device) {
        throw new BadRequestException(
          'iotDeviceId does not exist on production backend',
        );
    ...
    if (typeof iotMeasurementTypeId !== 'undefined') {
      const measurementType =
        await this.productionBackend.getMeasurementTypeById(
          Number(iotMeasurementTypeId),
        );
      if (!measurementType) {
        throw new BadRequestException(
    ...
    if (typeof measurementPosition === 'number') {
      const position =
        await this.productionBackend.getPositionById(measurementPosition);
      if (!position) {
        throw new BadRequestException(
          'measurementPosition does not exist on production backend',
        );
    ...
    const updated = await this.prisma.stageDeviceMapping.update({
      ...
      data: {
        ...
        ...(typeof measurementPosition === 'number'
          ? { measurementPosition }
          : {}),
  ...
  async ensureDeviceMappingExist(stageDeviceMappingId: number) {
    const foundStageDeviceMapping =
      await this.prisma.stageDeviceMapping.findUnique({
        where: {
