...
export class ProductionLineService {
  ...
  private async invalidateAllCache(): Promise<void> {
    try {
      ...
      this.logger.error(
        'Failed to bump production line cache version',
        error?.stack,
      );
    }
  }
  ...
  async create(dto: CreateProductionLineDTO) {
    ...
    if (!workshop) {
      throw new NotFoundException(
        `Workshop with id ${dto.workshopId} not found`,
      );
    }

    if (!workshop.isActive) {
      throw new BadRequestException(
        'Cannot create production line in a disabled workshop',
      );
    }
    ...
    if (existingLine) {
      throw new BadRequestException(
        `Production line code ${dto.code} already exists in this workshop`,
      );
    }
  ...
  async findAll(params?: { workshopId?: number; isActive?: boolean }) {
    ...
    const shouldUseCache =
      (!hasExplicitIsActive || effectiveIsActive === true) &&
    ...
    if (shouldUseCache) {
      try {
        ...
        this.logger.error(
          'Failed to read production lines from cache',
          error?.stack,
        );
      }
    }
    ...
    if (shouldUseCache && cacheKey) {
      try {
        ...
        this.logger.error(
          'Failed to write production lines to cache',
          error?.stack,
        );
      }
    }
  ...
  async update(id: number, dto: UpdateProductionLineDTO) {
    ...
    if (!workshop.isActive) {
      throw new BadRequestException(
        'Cannot update production line to a disabled workshop',
      );
    }
    ...
    if (dto.code || dto.workshopId) {
      ...
      if (duplicated) {
        throw new BadRequestException(
          `Production line code ${codeToCheck} already exists in this workshop`,
        );
      }
    }
