...
export class WorkshopService {
    ...
    private async invalidateAllCache(): Promise<void> {
        try {
            ...
            this.logger.error('Failed to bump workshop cache version', (error as any)?.stack);
        }
    }
    ...
    async findAll(params?: { isActive?: boolean }) {
        ...
        if (shouldUseCache) {
            try {
                ...
                this.logger.error('Failed to read workshops from cache', (error as any)?.stack);
            }
        }
        ...
        if (shouldUseCache && cacheKey) {
            try {
                ...
                this.logger.error('Failed to write workshops to cache', (error as any)?.stack);
            }
        }
