...
import { StageAssignmentService } from 'src/stage-assignment/stage-assignment.service';
...
export class ProductionPlanService {
  ...
  constructor(
    ...
    private readonly stageAssigmentService: StageAssignmentService,
  ) {}
  ...
  async create(dto: CreateProductionPlanDTO & { username: string }) {
    ...
    if (existingCode) {
      throw new BadRequestException(
        `Production plan code "${planCode}" already exists`,
      );
    }
    console.log('foundBrickType:', brickTypeId);

    const foundBrickType =
      await this.brickTypeService.ensureExists(brickTypeId);
    // if (!foundBrickType.isActive) {
    //     throw new BadRequestException(`Brick type with id ${brickTypeId} is inactive`);
    // }

    const foundProductionLine =
      await this.productionLineService.ensureExists(productionLineId);
    if (!foundProductionLine.isActive) {
      throw new BadRequestException(
        `Production line with id ${productionLineId} is inactive`,
      );
    }
  ...
  async update(id: number, dto: UpdateProductionPlanDTO) {
    ...
    if (dto.productionLineId) {
      const line = await this.productionLineService.ensureExists(
  ...
  async reject(id: number, username: string, reason?: string) {
    ...
    if (
      ...
      throw new BadRequestException(
        'Only DRAFT or APPROVED plans can be rejected',
      );
    }
  ...
  async markInProgress(id: number) {
    ...
    if (existing.status !== PlanStatus.APPROVED) {
      throw new BadRequestException(
        'Only APPROVED plans can be marked as IN_PROGRESS',
      );
    }

    //check if any stage asignment of current plans exist
    const existStageAssignment = await this.stageAssigmentService.findActive({
      productionPlanId: id,
    });
    if (!existStageAssignment.length)
      throw new BadRequestException(
        'There are any stage was assignment to this plans',
      );
  ...
  async markCompleted(id: number) {
    ...
    if (existing.status !== PlanStatus.IN_PROGRESS) {
      throw new BadRequestException(
        'Only IN_PROGRESS plans can be marked as COMPLETED',
      );
    }
  ...
  async markCancelled(id: number, reason?: string) {
    ...
    if (
      ...
      throw new BadRequestException(
        'Only DRAFT/APPROVED/IN_PROGRESS plans can be cancelled',
      );
    }
  ...
  async ensureExistByCode(planCode: string) {
    ...
    if (!existing) {
      throw new NotFoundException(
        `Production plan with code ${planCode} not found`,
      );
    }
    return existing;
  }
