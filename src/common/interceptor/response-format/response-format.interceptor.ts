...
import { ActivitySeverity, ActivitySource } from 'src/activity-log/activity-log.enum';
...
export class ResponseFormatInterceptor implements NestInterceptor {
    constructor(private readonly activityLogProviderService: ActivityLogProviderService) { }
        ...
        return next.handle().pipe((
            map((body) => {
                ...
                const payload: OkResponse = {
                    status: 'ok',
                    data: hasData ? (body as any).data : body,
                    ...(hasMeta ? { meta: (body as any).meta } : {}),
                }
